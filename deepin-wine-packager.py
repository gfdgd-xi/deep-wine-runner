...
def AddTab():
    ...
    e8_text.setWhatsThis("""填写该软件的中文或英文名称。""")
    e9_text.setWhatsThis("""图标只支持PNG格式和SVG格式，其他格式无法显示。""")
    e15_text.setWhatsThis("程序参数，如%u，一般不需要")
...
# 创建控件
e2_text.setWhatsThis("""6.5.50（随便填写或填写该软件的windows版本的版本号，6.5.50只是示例）。""")
e3_text.setWhatsThis("""随便填写或使用该软件的windows版本的软件简介。""")
e4_text.setWhatsThis("""填写自己的网名，若是自用软件，不上架至应用商店，不进行后续维护，可随便填写。""")
e5_text.setWhatsThis(f"<p>解压容器到其它机器的容器名称，一般自动带出</p><p><img src='{programPath}/Icon/Screen/202211121646232464_image.png'></p>")
e6_text.setWhatsThis(f"要打包的容器所在路径，也可以选择已经好打包的 7z 文件，一般自动带出")
e7_text.setWhatsThis("""可执行文件的运行路径格式是“C:/XXX/XXX.exe”（不包含引号）""")
debArch.setWhatsThis("选择生成 deb 包所对应的架构")
rmBash.setWhatsThis("清理容器无用内容，一般建议勾选，最新版本默认勾选，如果有特殊需求（如容器内有 mono、gecko 等）建议取消勾选")
debDepends.setWhatsThis("生成 deb 包所需的依赖，一般情况下默认即可")
debRecommend.setWhatsThis("生成 deb 包的推荐依赖，一般情况下为空即可")
cleanBottonByUOS.setWhatsThis("清理容器无用内容，一般建议勾选，最新版本默认勾选，如果有特殊需求（如容器内有 mono、gecko 等）建议取消勾选")
chooseWineHelperValue.setWhatsThis("使用星火 dwine helper 替换 Deepin Wine Helper，投稿星火应用商店的话建议勾选，最新版本默认勾选（如果打包 arm 包将不会提供选择）")
option1_text.setWhatsThis("""点击右侧的下拉箭头，选择该软件所属的软件分类即可，常见软件分类名称释义：
Network=网络应用；
Chat=即时通讯或社交沟通；
Video=视频播放；
...
e8_text.setWhatsThis("""填写该软件的中文或英文名称。""")
e9_text.setWhatsThis("""图标只支持PNG格式和SVG格式，其他格式无法显示。""")
e10_text.setWhatsThis("快捷方式的 MimeType 项，一般为空即可")
option1_text.setWhatsThis("打包的 Wine 版本，根据实际情况选择（如果打包 arm 包将不会提供选择）")
e12_text.setWhatsThis("打包出的 deb 生成的位置，一般自动生成")
e15_text.setWhatsThis("程序参数，如%u，一般不需要")
build7z.setWhatsThis("只打包容器生成 7z 包，不做其它操作")
buildDebDir.setWhatsThis("构建 deb 包目录，但不打包成 deb")
textbox1.setWhatsThis("查看打包过程中命令返回内容")
button5.setWhatsThis("点击该按钮打包生成 deb")
installDeb.setWhatsThis("调用默认的 deb 安装工具安装生成的 deb")
